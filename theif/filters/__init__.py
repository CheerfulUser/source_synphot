...
class filter(spectrum):
   ...
   def response(self, specwave, flux=None, z=0, zeropad=1, photons=1):
   ...
   def synth_mag(self, specwave, flux=None, z=0, zeropad=1):
      ...
      if res <= 0:
         print('no response')
         plt.figure()
         plt.plot(self.wave,self.resp/num.nanmedian(self.resp))
         plt.plot(specwave,flux/num.nanmedian(flux))
         return(num.nan)
      else:
